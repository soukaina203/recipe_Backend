...
node {
  def apps = [
    [name: "dish-api", port: '5000', exposed_port: '', path: '', domaine: "dish-api.cloud.dev-solus.com", volume: "/wwwroot",domaine_prefix: ''],
  ];
  ...
  stage('Cloning Git') {
   ...
   def currentCommit = commit.GIT_COMMIT // sh(script: "git rev-parse HEAD", returnStdout: true).trim()
    ...
    apps.each { e ->
      ...
      app = docker.build("${e.name}", "-t ${e.name} -f ./${e.path}/dockerfile ./")
